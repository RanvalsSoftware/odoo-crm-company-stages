...
reviewed upstream 17.0 views. They are not a copy of a live combined view.
...
FIXTURES = {
    "crm.crm_stage_form": """<form><sheet><div class="oe_title"><field name="name"/></div>
        <group><group><field name="fold"/><field name="color"/><field name="team_id"/></group>
    ...
    "crm.crm_stage_tree": """<tree multi_edit="1"><field name="sequence"/><field name="name"/>
        <field name="is_won"/><field name="team_id"/><field name="rotting_threshold_days"/></tree>""",
    "crm.crm_lead_stage_search": """<search><field name="name"/><field name="sequence"/>
        <field name="is_won"/><field name="team_id"/></search>""",
...
def main():
    ...
    for path in xml_files:
        ...
        for rec in tree.xpath("//record"):
            ...
            if inherit_id == "crm.crm_stage_form":
                ...
                assert len(company) == 1 and company[0].getnext().get("name") == "team_id"
    ...
    for company in [False, 10, 20]:
        for team in [False, 100, 200]:
            ...
            for stage_company in [False, 10, 20, 30]:
                for stage_teams in [False, 100, 200, 300]:
                    record = {"company_id": stage_company, "team_id": stage_teams}
                    expected = (not stage_company or stage_company == company) and (
                        not stage_teams or team == stage_teams
                    )
    ...
    assert len(group.args.args) == 4
    source = (ROOT / "models/crm_lead.py").read_text()
    assert ("from odoo.fields import Domain" in source) is False
    result = {
        "version": "17.0",
