...
class TestStageCompany(TransactionCase):
    ...
    def test_22_empty_kanban_columns_are_company_filtered(self):
        ...
        columns = lead_model._read_group_stage_ids(lead_model.env["crm.stage"], [])
    ...
    def test_23_group_expansion_cannot_reintroduce_foreign_stage(self):
        ...
        columns = lead_model._read_group_stage_ids(supplied, [])
        self.assertIn(self.stage_a, columns)
        self.assertNotIn(self.stage_b, columns)
    ...
    def test_24_group_expansion_keeps_native_team_restrictions(self):
        ...
        restricted = self.Stage.create({
            ...
            "team_ids": [Command.set(other_team.ids)],
        })
        ...
        columns = lead_model._read_group_stage_ids(restricted, [])
    ...
    def test_29_private_stage_rejects_foreign_team(self):
        with self.assertRaises(ValidationError), self.cr.savepoint():
            self.stage_a.write({"team_ids": [Command.set(self.team_b.ids)]})

    def test_30_shared_stage_can_restrict_multiple_company_teams(self):
        self.shared_stage.write({"team_ids": [Command.set((self.team_a | self.team_b).ids)]})
        self.assertEqual(self.shared_stage.team_ids, self.team_a | self.team_b)

    def test_31_team_company_change_cannot_invalidate_stage(self):
        self.stage_a.write({"team_ids": [Command.set(self.team_a.ids)]})
        with self.assertRaises(ValidationError), self.cr.savepoint():
    ...
    def test_35_stage_edit_modal_has_company_field(self):
        ...
        self.assertEqual(fields[0].getnext().get("name"), "team_ids")
