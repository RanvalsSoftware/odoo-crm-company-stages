...
class CrmLead(models.Model):
    ...
    def _read_group_stage_ids(self, stages, domain, order):
        ...
        expanded = super()._read_group_stage_ids(stages, domain, order)
        stage_domain = [
            ("id", "in", expanded.ids),
            ("company_id", "in", self.env.companies.ids + [False]),
        ]
        return self.env["crm.stage"].search(stage_domain, order=order or stages._order)
    ...
    def _stage_find(self, team_id=False, domain=None, order="sequence, id", limit=1):
        ...
        stage_domain = expression.AND([domain or [], [("company_id", "in", permitted)]])
